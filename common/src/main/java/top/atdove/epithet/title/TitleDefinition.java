...
public record TitleDefinition(
     ...
     * 2) Plain string: resolves gradient and rainbow markup (<gradient:...>, <animated-gradient:...>, <rainbow>),
     *    or falls back via translatableWithFallback(str, str) for i18n and literals.
     */
    public static final Codec<Component> FLEXIBLE_COMPONENT_CODEC = Codec.either(
        ...
        either -> either.map(
            str -> ColorHelper.hasGradientTags(str) ? Component.literal(str) : Component.translatableWithFallback(str, str),
            comp -> comp
        ),
        Either::right
    );
    ...
    /**
     * Resolves the display name with gradient/rainbow styling and fallback color styling.
     * Re-evaluates animated gradients dynamically based on current frame time.
     */
    public Component getFormattedDisplayName() {
        String raw = this.displayName.getString();
        if (ColorHelper.hasGradientTags(raw)) {
            return ColorHelper.parseGradientComponent(raw);
        }
