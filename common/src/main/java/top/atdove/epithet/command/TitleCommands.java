...
public class TitleCommands {
        ...
        dispatcher.register(
            Commands.literal("epithet")
                ...
                .then(Commands.literal("admin")
                    ...
                    // admin create <id> <name_json_or_text> [color] [desc] OR admin create json <json>
                    .then(Commands.literal("create")
                        .then(Commands.literal("json")
                            .then(Commands.argument("json", StringArgumentType.greedyString())
                                .executes(TitleCommands::executeAdminCreateJson)
                            )
                        )
                    ...
                    // admin set <targets> <id> OR admin set <targets> json <json>
                    .then(Commands.literal("set")
                        .then(Commands.argument("targets", EntityArgument.players())
                            .then(Commands.literal("json")
                                .then(Commands.argument("json", StringArgumentType.greedyString())
                                    .executes(TitleCommands::executeAdminSetJson)
                                )
                            )
                            .then(Commands.argument("id", ResourceLocationArgument.id())
                                .suggests((ctx, builder) -> SharedSuggestionProvider.suggestResource(TitleRegistry.getInstance().getAllTitles().keySet(), builder))
    ...
    private static int executeAdminCreate(CommandContext<CommandSourceStack> ctx, int color, String desc) {
        ...
        // If executed by an in-game player, automatically grant and equip the newly created title for instant feedback
    ...
    private static int executeAdminCreateJson(CommandContext<CommandSourceStack> ctx) {
        String jsonStr = StringArgumentType.getString(ctx, "json");
        try {
            TitleDefinition newTitle = TitleDefinition.CODEC.parse(JsonOps.INSTANCE, JsonParser.parseString(jsonStr))
                    .getOrThrow(err -> new IllegalArgumentException("Invalid title JSON schema: " + err));
            ...
            ctx.getSource().sendSuccess(() -> Component.translatable("command.epithet.admin.create.success", newTitle.id().toString()).withStyle(ChatFormatting.GREEN), true);
            return 1;
        } catch (Exception e) {
            ctx.getSource().sendFailure(Component.literal("Failed to parse Title JSON: " + e.getMessage()).withStyle(ChatFormatting.RED));
            return 0;
        }
    }

    private static int executeAdminSetJson(CommandContext<CommandSourceStack> ctx) throws CommandSyntaxException {
        Collection<ServerPlayer> targets = EntityArgument.getPlayers(ctx, "targets");
        String jsonStr = StringArgumentType.getString(ctx, "json");
        try {
            TitleDefinition newTitle = TitleDefinition.CODEC.parse(JsonOps.INSTANCE, JsonParser.parseString(jsonStr))
            ...
            for (ServerPlayer target : targets) {
                ...
                data.setActiveTitle(Optional.of(newTitle.id()));
                TitleNetworkHandler.syncToPlayerAndTrackers(target);
                target.sendSystemMessage(Component.translatable("command.epithet.admin.set.target", newTitle.displayName()).withStyle(ChatFormatting.GREEN));
            }

            ctx.getSource().sendSuccess(() -> Component.translatable("command.epithet.admin.set.sender", targets.size(), newTitle.id().toString()).withStyle(ChatFormatting.GREEN), true);
            return targets.size();
        } catch (Exception e) {
            ctx.getSource().sendFailure(Component.literal("Failed to parse Title JSON: " + e.getMessage()).withStyle(ChatFormatting.RED));
            return 0;
